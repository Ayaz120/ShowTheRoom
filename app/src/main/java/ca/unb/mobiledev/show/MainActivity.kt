...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        resetZoomButton.setOnClickListener {
            // Reset zoom logic
            mapImageView.scaleType = ImageView.ScaleType.CENTER_INSIDE
        ...
        floorSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parentView: AdapterView<*>?, view: View?, position: Int, id: Long) {
                currentFloorTextView.text = "Current Floor: ${floorSpinner.selectedItem}"
                // Logic to update the map based on the selected floor
            }
    ...
    private fun getDirectionsDataFromServer(start: String, end: String): List<Pair<String, List<String>>> {
        return when {
            start.equals("Entrance", ignoreCase = true) && end.equals("Building 1", ignoreCase = true) -> {
                listOf(
                    "Building 1" to listOf("Step 1: Go straight", "Step 2: Turn left", "Step 3: Arrive at Building 1")
                )
            }
            start.equals("Entrance", ignoreCase = true) && end.equals("Building 2", ignoreCase = true) -> {
                listOf(
                    "Building 2" to listOf("Step 1: Go right", "Step 2: Turn left", "Step 3: Arrive at Building 2")
                )
            }
            else -> {
